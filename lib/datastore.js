...
exports.route = function (app) {
  ...
  app.post('/answer', function(req, res, next) {
    ...
    if (req.body.content && (req.body.content = _.trim(req.body.content))) {
      Answer.findBy(req.conn, req.tx, 'content', req.body.content, function (aObj) {
        if(aObj){
        ...
        allQuestionOrders.list(req.tx, function (qorders) {
          Step(function() {
            ...
            res.render("quiz", {
               ...
               */
              locals: { quiz: qObj, questionOrders: qorders, answerOrders: aorderObjs }
            });
          });
        });
      } else {
